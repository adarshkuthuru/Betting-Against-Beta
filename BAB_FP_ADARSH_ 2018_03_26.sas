/*************************************************************************/
/***                           CODE SUMMARY                            ***/
/*** REPLICATION OF BETTING AGAINST BETA PAPER BY FRAZZINI AND PEDERSEN***/
/*************************************************************************/
*STEP-1: ESTIMATING EX-ANTE BETAS USING CRSP STOCK-LEVEL DATA;
*STEP-2: CONSTRUCTING NYSE BREAKPOINTS;
*STEP 3: TABLE-3 RESULTS FOR NYSE PORTFOLIOS; 
*STEP 4: TABLE-3 RESULTS FOR BAB FACTOR; 
/*************************************************************************/
...
/*proc download data=final out=final; run;*/
...
/* Step 4.1. BAB factor returns estimation */
proc sort data=final1; by daten beta; run;

*Ranking all stocks by date, beta;
proc rank data=final1 out=final2;
  by daten;
  var beta;
  ranks Rank_all;
run;

*Calcualting mean rank by month and add it back to final2;
proc sql;
	create table final2 as
	select distinct *,mean(Rank_all) as mean_rank
	from final2
	group by daten;
quit;

*Estimating the normalizing constant 'k';
proc sql;
	create table final2 as
	select distinct *,2/sum(abs(sum(Rank_all,-mean_rank))) as k
	from final2
	group by daten;
quit;

* Calculating the weights of each stock;
proc sql;
	create table final2 as
	select distinct *,abs(sum(rank_all,-mean_rank))*k as wt
	from final2
	order by daten,rank_bab,wt;
quit;

*To check if total weights of low and high P/F's are adding upto 1;
/*proc sql;*/
/*	create table final3 as*/
/*	select distinct daten,rank_bab,sum(wt)*/
/*	from final2*/
/*	group by daten,rank_bab;*/
/*quit;*/

*Calculating P/F (High beta & Low beta) returns and betas;
proc sql;
	create table final3 as
	select distinct daten,rank_bab,sum(-rf,sum(wt*ret))/sum(wt*beta) as rp
	from final2
	group by daten,rank_bab;
quit;

*converting long to wide form;
proc transpose data=final3 out=final3 prefix=Rank;
    by daten; *only one year per column;
    id Rank_bab; *will be split to columns;
/*    var Mean_ret; *by this variable;*/
run;
...
data final3;
  set final3;
  Ret_BAB=sum(Rank1,-Rank2);
run;

/* Step 4.2. Table-3 results for BAB factor */
/* Step 4.2.1. estimating excess ret for BAB portfolio */
proc sql;
	create table final3 as
	select distinct a.*,b.*
	from final3 as a, bb.ff_monthly as b
	where month(a.daten)=month(b.dateff) and year(a.daten)=year(b.dateff);
quit;

data final3;
  set final3;
  ExRet_BAB=sum(Ret_BAB,-rf);
  Exmkt=sum(mktrf,-rf);
run;

proc means data=final3 noprint;
  var ExRet_BAB;
  output out=exret_bab mean=Mean_exret t=t;
quit;
...
proc transpose data=exret_bab out=exret_bab;
  by _TYPE_ notsorted;
  var Mean_exret t;
run;
...
/* Step 4.2.2. CAPM alpha */
proc reg data=final3 noprint tableout outest=Alpha;
  model ExRet_BAB = Exmkt;
quit;
...
data Alpha;
  ...
  keep Rank_beta _Type_ Intercept;
  if _Type_='PARMS' then Intercept=Intercept*100;
  rename Intercept=BAB;
run;

proc reg data=final3 noprint tableout outest=Alpha1;
  model ExRet_BAB = Exmkt SMB HML;
quit;
...
/* Step 4.2.3. Three-factor alpha */
proc reg data=final3 noprint tableout outest=Alpha2;
  model ExRet_BAB = Exmkt SMB HML UMD;
quit;
...
/* Step 4.2.4. Four-factor alpha */
proc reg data=final3 noprint tableout outest=Alpha3;
  model ExRet_BAB = Exmkt SMB HML UMD liquidity;
quit;
...
/* Step 4.2.5. Beta-realized */
proc reg data=final3 noprint tableout outest=beta;
  model ExRet_BAB = Exmkt;
quit;
...
/* Step 4.2.6. Volatility */
proc means data=final3 noprint;
  var ExRet_BAB;
  output out=vol_bab std=std;
quit;
...
run;
 ...
 Step 4.2.7. Sharpe ratio 
proc means data=final3 noprint;
  var ExRet_BAB;
  output out=SR_bab mean=mean std=std;
quit;
...
proc append data=exret_bab base=Stats1; run;
proc append data=alpha force base=Stats1; run;
proc append data=alpha1 force base=Stats1; run;
proc append data=alpha2 force base=Stats1; run;
proc append data=alpha3 force base=Stats1; run;
proc append data=beta force base=Stats1; run;
proc append data=vol_bab force base=Stats1; run;
proc append data=SR_bab force base=Stats1; run;

proc download data=stats1 out=stats1; run; 

proc download data=final out=final; run;
proc download data=stats out=stats; run;
endrsubmit;
