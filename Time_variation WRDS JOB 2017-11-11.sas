...
data monthly;
	...
	if CALDT>='01Jan1980'd and CALDT<='30Sep2017'd;
...
proc sql;
	create table test1 as
	select distinct a.*,b.MKTRF as MKTRF_1
	from test1 as a left join ff.FACTORS_DAILY as b
	on intck('day',a.CALDT,b.date)=-1;
quit;

proc sql;
	create table test1 as
	select distinct a.*,b.MKTRF as MKTRF_2
	from test1 as a left join ff.FACTORS_DAILY as b
	on intck('day',a.CALDT,b.date)=-2;
quit;


proc sort data=test1; by year mon CALDT; run;
proc reg data=test1 noprint tableout outest=beta;
  by year mon;
  model Ex_MF_ret = MKTRF MKTRF_1 MKTRF_2;
quit;
data beta;
  set beta;
  where _Type_ in ('PARMS');
  keep mon year MKTRF MKTRF_1 MKTRF_2;
  rename MKTRF=Beta;
  rename MKTRF_1=Beta1;
  rename MKTRF_2=Beta2;
run;

data beta;
  set beta;
  Agg_Beta=sum(Beta,Beta1,Beta2);
  drop Beta Beta1 Beta2;
run;
